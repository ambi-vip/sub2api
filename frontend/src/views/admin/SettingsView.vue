            ...
            <div class="space-y-5 p-6">
              ...
              <template v-else>
                <div class="flex items-center justify-between gap-4">
                  ...
                  <Toggle v-model="opencodeGoUsageForm.enabled" data-testid="opencode-go-usage-global-enabled" />
                </div>
                <div v-if="opencodeGoUsageForm.enabled" class="space-y-4 border-t border-gray-100 pt-4 dark:border-dark-700">
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="opencode-go-usage-debounce">
                    {{ t("admin.settings.opencodeGoUsage.debounceMinutes") }}
                    <input id="opencode-go-usage-debounce" v-model.number="opencodeGoUsageForm.debounce_minutes" type="number" min="1" max="60" class="input mt-2 w-32" />
                  </label>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="opencode-go-usage-interval">
                    {{ t("admin.settings.opencodeGoUsage.intervalMinutes") }}
                    <input id="opencode-go-usage-interval" v-model.number="opencodeGoUsageForm.interval_minutes" type="number" min="5" max="1440" class="input mt-2 w-32" />
                  </label>
                </div>
                <div class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700">
                  <button type="button" class="btn btn-primary btn-sm" :disabled="opencodeGoUsageSaving" @click="saveOpenCodeGoUsageSettings">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div>
                ...
                <input v-model="form.claude_code_client_version" type="text" class="input w-full font-mono text-sm" placeholder="2.1.258" />
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
              ...
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p v-if="claudeSyncedVersionLabel" class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
...
const claudeSyncedVersionLabel = computed(() => {
  ...
  if (!synced) return "";
  return t("admin.settings.gatewayForwarding.claudeCodeVersionSyncedValue", { version: synced });
});
...
async function loadOpenCodeGoUsageSettings() {
  ...
  try {
    ...
    // Keep fail-safe disabled defaults when the optional setting is unavailable.
  } finally {
