...
const (
	AccountTestModeModelTrace       = "modeltrace"
	accountTestModelTraceContextKey = "account_test_modeltrace"
...
func createTestPayload(modelID string, prompts ...string) (map[string]any, error) {
	...
	return createTestPayloadWithSystemPrompt(modelID, testPrompt, claudeCodeSystemPrompt)
}

func createAccountTestPayload(c *gin.Context, modelID, prompt string) (map[string]any, error) {
	systemPrompt := claudeCodeSystemPrompt
	if c != nil {
		if modelTrace, _ := c.Get(accountTestModelTraceContextKey); modelTrace == true {
			systemPrompt = ""
		}
	}
	return createTestPayloadWithSystemPrompt(modelID, prompt, systemPrompt)
}

func createTestPayloadWithSystemPrompt(modelID, testPrompt, systemPrompt string) (map[string]any, error) {
	maxTokens := 1024
	if strings.TrimSpace(testPrompt) != "" && testPrompt != "hi" {
		maxTokens = 4096
	}
...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string, opts ...AccountTestOptions) error {
	...
	if mode == AccountTestModeModelTrace {
		c.Set(accountTestModelTraceContextKey, true)
	}
...
func (s *AccountTestService) testOpenCodeGoResponsesConnection(c *gin.Context, account *Account, testModelID string, prompts ...string) error {
	...
	c.Writer.Flush()
	s.sendEvent(c, TestEvent{Type: "test_start", Model: testModelID})
	return s.testCNProviderAdaptiveResponsesConnection(c, account, testModelID, authToken, firstPrompt(prompts))
}
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string, prompts ...string) error {
	...
	payload, err := createAccountTestPayload(c, testModelID, prompt)
	if err != nil {
...
func (s *AccountTestService) testClaudeVertexServiceAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string, prompts ...string) error {
	...
	payload, err := createAccountTestPayload(c, testModelID, firstPrompt(prompts))
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create test payload")
	}
	payloadBytes, _ := json.Marshal(payload)
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID string, prompts ...string) error {
	...
	payloadBytes, err := buildGrokQuotaProbeBody(testModelID, firstPrompt(prompts))
	if err != nil {
...
func (s *AccountTestService) testAntigravityAccountConnection(c *gin.Context, account *Account, modelID string, prompts ...string) error {
	...
	result, err := s.antigravityGatewayService.TestConnection(ctx, account, testModelID, firstPrompt(prompts))
	if err != nil {
		return s.sendErrorAndEnd(c, err.Error())
	}
...
func (s *AccountTestService) RunTestBackground(ctx context.Context, accountID int64, modelID string) (*ScheduledTestResult, error) {
	return s.runTestPromptBackground(ctx, accountID, modelID, "", AccountTestModeDefault)
}

// RunTestPromptBackground executes a custom prompt through the same account-specific
// transport and authentication path as the interactive account test.
func (s *AccountTestService) RunTestPromptBackground(ctx context.Context, accountID int64, modelID string, prompt string) (*ScheduledTestResult, error) {
	return s.runTestPromptBackground(ctx, accountID, modelID, prompt, AccountTestModeModelTrace)
}

func (s *AccountTestService) runTestPromptBackground(ctx context.Context, accountID int64, modelID string, prompt string, mode string) (*ScheduledTestResult, error) {
	startedAt := time.Now()
